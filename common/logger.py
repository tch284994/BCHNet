...
def count_params(model):
    ...
    return learnable_params, total_params
...
class AverageMeter:
    ...
    def __init__(self, dataset):
        ...
        if self.benchmark == 'pascal':
            ...
            self.class_ids_interest = torch.tensor(self.class_ids_interest).cuda()
            self.nclass = 7
        elif self.benchmark == 'verse2D_axial':
            self.class_ids_interest = dataset.class_ids
            self.class_ids_interest = torch.tensor(self.class_ids_interest).cuda()
            self.nclass = 1
        elif self.benchmark == 'verse2D_sagittal':
            self.class_ids_interest = dataset.class_ids
            self.class_ids_interest = torch.tensor(self.class_ids_interest).cuda()
            self.nclass = 1
...
class Logger:
    ...
    def initialize(cls, args, training):
        ...
        logpath = args.logpath if training else '_TEST_' + args.load.split('/')[-2].split('.')[0] + logtime + "_" + str(args.nshot) + "shot_" + args.benchmark
        if logpath == '':
            logpath = logtime
        ...
        # Log arguments
        logging.info('\n:=========== Cross-Domain Few-shot Seg. via BCHNet ===========')
    ...
    @classmethod
    def save_domain_adaptation_model(cls, model, epoch, val_miou):
        torch.save(model.state_dict(), os.path.join(cls.logpath, str(epoch) + 'best_model.pt'))
        cls.info('Model saved @%d w/ val. mIoU: %5.2f.\n' % (epoch, val_miou))
