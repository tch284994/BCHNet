...
def train(epoch, model, dataloader, optimizer, training):
    ...
    for idx, batch in enumerate(train_bar):
        ...
        # batch['support_imgs'].shape: [bsz, n-shot, c, h, w]
        # batch['query_img'].shape:    [bsz, c, h, w]
        ...
        loss = model.module.compute_objective(logit_mask_1, batch['query_mask']) + 0.5 * model.module.pred_mask_loss(mask_rough, batch['query_mask']) + model.module.compute_objective(logit_mask_2, batch['support_masks'])
        if training:
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logpath', type=str, default='')  # default=''
    parser.add_argument('--bsz', type=int, default=12)      # default=12
    parser.add_argument('--lr', type=float, default=1e-3)   # default=1e-3
    parser.add_argument('--niter', type=int, default=30)    # default=30 (num of epochs)
    ...
    for epoch in range(args.niter):
        print("Epoch:{}/{}  Lr:{:.2E}".format(epoch, args.niter,optimizer.state_dict()['param_groups'][0]['lr']))
